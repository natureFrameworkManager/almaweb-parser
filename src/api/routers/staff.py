...
@router.get("/distinct/fields", summary="Get distinct values for a staff field")
def get_staff_distinct_field(
    session: SessionDep,
    field_name: Annotated[dict, Depends(distinct_parameters(Staff))],
    paging: Annotated[dict, Depends(paging_parameters)],
    export: Annotated[dict, Depends(export_parameters)],
):
    ...
    query = select(getattr(Staff, field)).distinct()  # type: ignore
    if order:
        sort_column = getattr(Staff, field)  # type: ignore
